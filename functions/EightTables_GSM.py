...
with tqdm(total=total_rows, desc="Elaborazione complessiva") as pbar:
    ...
    for sheet_name in reversed(excel_data.sheet_names):
        ...
        def process_rows(values, is_channel=True, temp_output_df=pd.DataFrame()):
            for value in values:
                ...
                if not filtered_df.empty:
                    ...
                    # Se l'operatore banda è GSM, memorizza il valore
        ...
        temp_output_df = process_rows(unique_channels, is_channel=True, temp_output_df=temp_output_df)
        temp_output_df = process_rows(unique_arfcn_values, is_channel=False, temp_output_df=temp_output_df)
